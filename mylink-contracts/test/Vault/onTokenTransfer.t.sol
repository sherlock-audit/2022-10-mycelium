...
contract Vault_onTokenTransferTest is VaultTest {
    event Deposit(address indexed from, uint256 value);

    function testSingleTransfer(uint256 amount) public {
        vm.assume(amount > 0);
        ...
        link.transferAndCall(address(vault), amount, abi.encodePacked("deposit"));
    ...
    function testDepositZero() public {
        ...
        link.transferAndCall(address(vault), 0, abi.encodePacked("deposit"));
    }
    ...
    function testThreeDeposits(
        ...
        vm.startPrank(alice);
        link.transferAndCall(address(vault), aliceAmount, abi.encodePacked("deposit"));
        vm.stopPrank();
        ...
        vm.startPrank(bob);
        link.transferAndCall(address(vault), bobAmount, abi.encodePacked("deposit"));
        vm.stopPrank();
        ...
        vm.startPrank(carol);
        link.transferAndCall(address(vault), carolAmount, abi.encodePacked("deposit"));
        vm.stopPrank();
